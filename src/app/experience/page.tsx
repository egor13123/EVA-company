    ...
    <>
      ...
      <section className="container-x">
        <div className="grid lg:grid-cols-2 gap-px bg-line border border-line">
          <Reveal as="article" className="bg-paper p-8 md:p-20 relative">
            <span className="frame-corner tl text-ink" />
            <span className="frame-corner br text-ink" />
            <p className="eyebrow">Участие</p>
            <p className="display text-[clamp(3.5rem,14vw,11rem)] mt-6 md:mt-8 leading-[0.9]">
              200+
            </p>
            <p className="mt-6 md:mt-8 text-base md:text-xl text-ink/80 leading-relaxed max-w-md">
              контрактов, в которых мы приняли участие как поставщик в&nbsp;сферах
              торговли и&nbsp;строительства.
            </p>
          </Reveal>

          <Reveal as="article" className="bg-paper p-8 md:p-20 relative" delay={0.1}>
            <span className="frame-corner tl text-ink" />
            <span className="frame-corner br text-ink" />
          ...
          <Reveal as="article" className="bg-paper p-8 md:p-20 relative" delay={0.2}>
            <span className="frame-corner tl text-ink" />
            <span className="frame-corner br text-ink" />
          ...
          <Reveal as="article" className="bg-paper p-8 md:p-20 relative" delay={0.3}>
            <span className="frame-corner tl text-ink" />
            <span className="frame-corner br text-ink" />
      ...
      <section className="container-x mt-20 md:mt-48">
        ...
        <Reveal delay={0.05}>
          <h2 className="display text-[1.875rem] md:text-5xl mt-5 md:mt-6 max-w-[26ch] leading-[1.15] md:leading-[1.05]">
            Опыт поставок для&nbsp;крупных строительных и&nbsp;инфраструктурных
            организаций
          </h2>
        </Reveal>
        <Reveal delay={0.1}>
          <p className="mt-6 md:mt-8 text-base md:text-lg text-ink/80 leading-relaxed max-w-3xl">
        ...
        <div className="mt-10 md:mt-12 grid sm:grid-cols-2 gap-px bg-line">
          {[
            ...
            <Reveal
              key={client.name}
              delay={i * 0.06}
              className="bg-paper p-7 md:p-12"
            >
              <span className="text-[11px] uppercase tracking-widest text-ash">
                0{i + 1}
              </span>
              <div className="mt-5 md:mt-6 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-6 md:gap-8">
                <p className="display text-xl md:text-3xl leading-[1.2] flex-1">
                  {client.name}
                </p>
                ...
                <img
                  src={client.logo}
                  alt=""
                  className="block shrink-0 h-14 w-36 sm:h-16 sm:w-28 md:h-24 md:w-44 object-contain [filter:grayscale(100%)] opacity-80"
                />
              </div>
            </Reveal>
          ))}
